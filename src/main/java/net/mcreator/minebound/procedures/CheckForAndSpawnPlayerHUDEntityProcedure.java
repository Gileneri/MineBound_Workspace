				...
				foundTarget = false;
				{
					...
					for (Entity entityiterator : _entfound) {
						if (entityiterator instanceof PlayerHeadHUDEntityEntity) {
							if ((entityiterator.getDisplayName().getString()).equals(entity.getDisplayName().getString()) || (entityiterator.getDisplayName().getString()).equals("jeb_") && (entity.getDisplayName().getString()).equals("Dev")) {
								foundTarget = true;
								break;
							}
						}
					...
					if ((entity.getDisplayName().getString()).equals("Dev")) {
						if (world instanceof ServerLevel _level)
							_level.getServer().getCommands().performPrefixedCommand(
									...
									("summon minebound:player_head_hud_entity ~ ~ ~ {CustomName:'[{\"text\":\"" + "jeb_"
											...
											+ "jeb_" + "},Count:1}],ArmorDropChances:[0f,0f,0f,0.00f]}"));
					} else {
						if (world instanceof ServerLevel _level)
							_level.getServer().getCommands().performPrefixedCommand(
									new CommandSourceStack(CommandSource.NULL, new Vec3(x, (y + 1000000), z), Vec2.ZERO, _level, 4, "", Component.literal(""), _level.getServer(), null).withSuppressedOutput(),
			...
			if ((entity.getDisplayName().getString()).equals("Dev")) {
				if (world instanceof ServerLevel _level)
					_level.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, new Vec3(x, (y + 1000000), z), Vec2.ZERO, _level, 4, "", Component.literal(""), _level.getServer(), null).withSuppressedOutput(),
							("summon minebound:player_head_hud_entity ~ ~ ~ {CustomName:'[{\"text\":\"" + "jeb_"
									+ "\"}]',Invulnerable:1b,NoAI:1b,NoGravity:1b,PersistenceRequired:1b,Silent:1b,ActiveEffects:[{Id:14,Duration:99999999,Amplifier:0,ShowParticles:0b}],ArmorItems:[{},{},{},{id:player_head,tag:{SkullOwner:" + "jeb_"
