...
public class RuinedGasStationFeature extends Feature<NoneFeatureConfiguration> {
	public static RuinedGasStationFeature FEATURE = null;
	public static Holder<ConfiguredFeature<NoneFeatureConfiguration, ?>> CONFIGURED_FEATURE = null;
	public static Holder<PlacedFeature> PLACED_FEATURE = null;

	public static Feature<?> feature() {
		FEATURE = new RuinedGasStationFeature();
		CONFIGURED_FEATURE = FeatureUtils.register("minebound:ruined_gas_station", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("minebound:ruined_gas_station", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public RuinedGasStationFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("minebound", "gas_station1"));
		if (template == null)
			return false;
