...
public class RuinedBenchesSidewalkFeature extends Feature<NoneFeatureConfiguration> {
	public static RuinedBenchesSidewalkFeature FEATURE = null;
	public static Holder<ConfiguredFeature<NoneFeatureConfiguration, ?>> CONFIGURED_FEATURE = null;
	public static Holder<PlacedFeature> PLACED_FEATURE = null;

	public static Feature<?> feature() {
		FEATURE = new RuinedBenchesSidewalkFeature();
		CONFIGURED_FEATURE = FeatureUtils.register("minebound:ruined_benches_sidewalk", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("minebound:ruined_benches_sidewalk", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public RuinedBenchesSidewalkFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("minebound", "benches1"));
		if (template == null)
			return false;
		...
		if ((context.random().nextInt(1000000) + 1) <= 1500) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (!base_blocks.contains(context.level().getBlockState(new BlockPos(i, j, k)).getBlock()))
					continue;
				BlockPos spawnTo = new BlockPos(i + -16, j + -1, k + -8);
